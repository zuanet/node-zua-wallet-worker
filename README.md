...
Zua Wallet is implemented in TypeScript and can be used server-side
(NodeJs) and client-side (browser).

**PLEASE NOTE: This project is under heavy development**

## Installing zua-wallet-worker

```
npm install -g @zua/wallet-worker
```

## Cloning zua-wallet-worker

```
git clone https://github.com/zuanet/node-zua-wallet-worker
cd node-zua-wallet-worker
npm install
```
...
* [zua-grpc](https://github.com/zuanet/node-zua-grpc) - provides gRPC bindings for `zuad`
* [zua-grpc-node](https://github.com/zuanet/node-zua-grpc-node) - gRPC transport for server-side (NodeJs)
* [zua-grpc-web](https://github.com/zuanet/node-zua-grpc-web) - gRPC transport for client-side (browsers)
* [zua-core-lib](https://github.com/zuanet/node-zua-core-lib) - Zua UTXO and transaction data structures

Applications built on top of Zua Wallet Framework:

* [zua-faucet](https://github.com/zuanet/node-zua-faucet) - Zua faucet website
* [zua-wallet-cli](https://github.com/zuanet/node-zua-wallet-cli) - command-line wallet
* [KDX](https://github.com/zuanet/zua-kdx) - Zua desktop wallet

**PLEASE NOTE:** all Zua applications and libraries are under heavy
development.

## Zua Wallet Framework

Before you can use Zua Wallet, you need to initialize the framework.
...
synchronization. Wallet synchronization will connect to `zuad` and
...
`zuad` and dynamically update wallet UTXO entries as well as
balances.
...
* `networkFeeMax` - (optional) Allows you to set an upper bound for
  automatic network (data storage) fee calculation. Zua Wallet will
...
* *available* - balance contains ZUA ready to be spent, comprised of
...
* `blue-score-changed` - indicates Zua blue score change (new block
  generation)
